...
async function apiRequest<T>(
  ...
  if (!response.ok) {
    ...
    if (errorData.error) {
      ...
      if (errorData.code === "MISSING_DOCUMENTS") {
        ...
        if (docMatch && docMatch[1]) {
          ...
          errorMessage =
            "Please upload all required documents. Some documents are missing or not approved.";
        }
      } else if (errorData.code === "DOCUMENT_NOT_APPROVED") {
        errorMessage =
          "One or more documents are pending approval. Please wait for approval or upload new documents.";
      } else if (errorData.code === "INVALID_DOCUMENT") {
        errorMessage =
          "One or more documents are invalid. Please check and upload valid documents.";
      }
    ...
    // Check for error message in various formats (e.g. backend "error" field for 400)
    const errorMessage =
      errorData.error ||
      errorData.message ||
...
export const shipmentApi = {
  ...
  async getAll(params?: {
    ...
    if (params?.origin)
      searchParams.append(
        "origin",
        normalizeLocation(params.origin) ?? params.origin,
      );
  ...
  async create(data: CreateShipmentPayload): Promise<Shipment> {
    ...
    const normalizedData: CreateShipmentPayload = {
      ...
      destination:
        normalizedDestination as CreateShipmentPayload["destination"],
    };
  ...
  async getAcceptedShipItems(shipId: number): Promise<ShipItem[]> {
    return apiRequest<ShipItem[]>(
      `${API_PATH}/${shipId}/list-accepted-ship-items`,
    );
  },
  ...
  // Mark ship item as completed (PATCH /api/v1/ship-item/{id}/complete)
  async completeShipItem(shipItemId: number): Promise<string> {
    return apiRequest<string>(`/ship-item/${shipItemId}/complete`, {
      method: "PATCH",
    });
  },
};
