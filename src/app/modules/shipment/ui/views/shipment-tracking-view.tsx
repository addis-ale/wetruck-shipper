...
const getStatusBadgeVariant = (status: string) => {
    ...
    if (s.includes("completed") || s.includes("delivered")) return "default" as const;
    if (s.includes("accepted") || s.includes("allocated")) return "default" as const;
...
export function ShipmentTrackingView({ shipId }: ShipmentTrackingViewProps) {
    ...
    const { data: trackingData, isLoading: trackingLoading, error: trackingError } = useTrackShipment(shipId);
    ...
    const totalTrackedContainers = trackingItems.reduce(
        (acc, item) => acc + (item.ship_item?.containers?.length ?? 0), 0
    );
    const totalLocationLogs = trackingItems.reduce(
        (acc, item) => acc + (item.count_location_log ?? 0), 0
    );
                            ...
                            {STATUS_STEPS.map((step, idx) => {
                                ...
                                return (
                                    <div key={step.key} className="relative pb-6 last:pb-0">
                                        ...
                                        <div
                                            className={`absolute left-[-20px] top-1 flex h-5 w-5 items-center justify-center rounded-full border-2 ${isCurrent
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-3 px-4 pt-0">
                            {trackingItems.map((item) => (
                                <div key={item.truck_id} className="rounded-xl border bg-card p-4 shadow-sm">
                                    ...
                                    {item.ship_item?.containers?.length > 0 && (
                                        <div className="space-y-1.5">
                                            {item.ship_item.containers.map((c, idx) => (
                                                <div key={idx} className="flex items-center gap-2 rounded-lg bg-muted/40 px-3 py-2">
                                                    <ContainerIcon className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                                                    <span className="text-xs font-mono font-medium text-foreground">
                                                        {c.container_number}
                                                    </span>
                                                    <span className="text-[10px] text-muted-foreground">
                                                        {containerSizeLabel(c.container_size)} / {c.container_type}
                                                    </span>
                                                </div>
                                            ))}
                                        </div>
    ...
    return (
        <div className="space-y-6 pb-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <Badge variant={getStatusBadgeVariant(status)} className="text-sm uppercase font-medium px-3 py-1">
                    {status.replace(/_/g, " ")}
                </Badge>
            </div>
                        ...
                        {STATUS_STEPS.map((step, idx) => {
                            ...
                            return (
                                <div key={step.key} className="flex flex-col items-center flex-1 min-w-[80px] relative">
                                    ...
                                    {idx > 0 && (
                                        <div
                                            className={`absolute top-4 right-1/2 w-full h-0.5 -translate-y-1/2 ${idx <= currentStepIdx ? "bg-primary" : "bg-border"
                                                }`}
                                            style={{ left: "-50%" }}
                                        />
                                    )}
                                    ...
                                    {/* Step circle */}
                                    <div
                                        className={`relative z-10 flex h-8 w-8 items-center justify-center rounded-full border-2 transition-all ${isCurrent
                                    ...
                                    {/* Label */}
                                    <p
                                        className={`text-[11px] text-center mt-2 leading-tight ${isCurrent
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {trackingItems.map((item) => (
                                <div key={item.truck_id} className="rounded-xl border bg-card p-5 shadow-sm">
                                    ...
                                    {item.ship_item?.containers?.length > 0 && (
                                        <div className="space-y-2">
                                            ...
                                            <div className="space-y-1.5">
                                                {item.ship_item.containers.map((c, idx) => (
                                                    <div key={idx} className="flex items-center justify-between gap-2 rounded-lg bg-muted/40 px-3 py-2">
                                                        <div className="flex items-center gap-2">
                                                            <ContainerIcon className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
